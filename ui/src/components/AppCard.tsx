...
export default function AppCard({ app, source, onChanged }: AppCardProps) {
  ...
  return (
    <li className="va-card">
      <div className="va-card__top">
        <div className="va-card__icon" aria-hidden="true">
          {icon}
        </div>
      ...
      <div className="va-tags" aria-label="targets and scopes">
        {(app.products || []).map((p) => (
          <span key={p} className="va-tag va-tag--product">
            {p}
          </span>
        ))}
        {(app.scopes || []).map((s) => (
          <span key={s} className="va-tag">
            {s}
          </span>
        ))}
